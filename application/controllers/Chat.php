...
class Chat extends CI_Controller{
	...
	public function update()
	{
		...
		$st = $this->Chatmodel->insertMsg($this->session->userdata('username'), $message, $current->getTimestamp());
		var_dump($st);
...
//        }
		...
		if($query->num_rows()>0){
            for ($i=(count($data)-1); $i >= 0; --$i){
                ...
                $escmsg = htmlspecialchars(stripslashes($data[$i]['msg']));
                $escid = htmlspecialchars(stripslashes($data[$i]['id']));
                $escuser = htmlspecialchars(stripslashes($data[$i]['user']));
                echo "\t<message>\n";
                echo "\t\t<id>$escid</id>\n";
                echo "\t\t<author>$escuser</author>\n";
